    ...
    return (
      <div className="w-full sm:w-1/2 mt-3 md:w-1/3 px-3">
        <div className="card p-3 min-h-[150px] border border-black flex flex-col justify-between hover:bg-red-50 shadow-lg">
          ...
          <a href={data.link} target='_blank' className="border font-semibold transition-all duration-500 hover:bg-[red] hover:text-white border-yellow-300 rounded-full px-2 py-1 max-w-max">{data.btn}</a>
        </div>
      </div>
    )
  });
